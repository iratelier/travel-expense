var Datepicker = (function () {
  ...
  <div class="datepicker-picker">
    <div class="datepicker-header">
      ...
      <div class="datepicker-controls">
        ${getButtons(["prev-button", "view-switch", "next-button"])}
      </div>
    </div>
    <div class="datepicker-main"></div>
    <div class="datepicker-footer">
      <div class="datepicker-controls">
        ${getButtons(["today-button", "clear-button"])}
      </div>
    </div>
  </div>
</div>`);
  ...
  class DaysView extends View {
    ...
    render() {
      ...
      /* 수정 : 달력의 마지막 주까지만 표시하도록 42개의 셀을 마지막 주까지만 렌더링 */
      const totalDays = (this.last - this.start) / 86400000 + 1; // 마지막 날짜까지의 총 일수 계산
      const visibleDays = Math.ceil(totalDays / 7) * 7; // 주 단위로 표시할 일수 계산

      Array.from(this.grid.children).forEach((el, index) => {
        if (index >= visibleDays) {
          el.style.display = "none"; // 마지막 주 이후의 셀 숨김
          return;
        }
        el.style.display = ""; // 기존 셀의 스타일 초기화
